...
export const updateEarnedTime = createAsyncThunk<void, { userId: string; newTime: number }>(
  'user/updateEarnedTime',
...
export const updateCompletedTasks = createAsyncThunk<void, { userId: string; taskIds: string[] }>(
...
export const joinLeaderboard = createAsyncThunk<string, { userId: string; leaderboardId: string }>(
  'user/joinLeaderboard',
  async ({ userId, leaderboardId }, { rejectWithValue }) => {
    try {
      const response = await axios.put(`/api/user/${userId}/join-leaderboard`, { leaderboardId });
...
const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchEarnedTime.fulfilled, (state, action: PayloadAction<number>) => {
        state.status = 'succeeded';
